...
## Run
```
docker image build -t qgis-server-sidecar:v1.0.9 .
```
```
docker container run --rm --name qgis-server-sidecar \
      ...
      -e PRODUCT_TYPES='["dtm","dsm"]' \
      -v /docker/qgis/data:/io/data \
      qgis-server-sidecar:v1.0.9 -d
```
```
...
```
```
AWS_ENDPOINT_URL=http://10.8.0.9:9000 AWS_BUCKET_NAME=dem-int RAW_DATA_PROXY_URL=https://dem-int-proxy-production-nginx-s3-gateway-route-integration.apps.j1lk3njp.eastus.aroapp.io PRODUCT_TYPES='["dtm","dsm"]' npx zx script.mjs
```